...
module.exports = function ( mapper, reducer, index, cache )
{
    ...
    /**profile**/
    var rtime = 0, rcount = 0, rrows = 0, redfunc = reducer;
    reducer = reducer && function ( a, b, c )
    {
        rcount ++;
        rrows  += b.length;
        var t0  = Date.now (),
    ...
    self.add = function ( doc )
    {
        ...
        mapper ( doc, function ( k, v )
        {
            ...
            if ( chained && warm )
                self.query ({ key : k, group : true }).rows.forEach ( function ( row )
                {
                    if ( typeof row.key === 'string' )
                        chained.remove ({ _id : row.key, value : row.value });
                });
            ...
            if ( chained && warm )
                ...
                {
                    if ( typeof row.key === 'string' )
                        chained.add ({ _id : row.key, value : row.value });
                });
        });
    ...
    {
        /**profile**/ rcount = 0; rtime = 0; rrows = 0; cget = 0; cgett = 0; cput = 0; cputt = 0; /**/
        ...
        ranges.forEach ( function ( rows, i )
        {
            ...
            if ( reduce )
                ...
                out = out.concat ( rows.map ( function ( row )
                {
        ...
        return {
            ...
            output  : t3 - t2,
            total   : t3 - t0,
            reduce  : { count : rcount, time : rtime, rows : rrows },
            rcache  :
            {
    ...
    self.warmUp = function ()
    {
        ...
        delete self.warm;
        var rows = self.query ({ group : true }).rows,
            i, n = rows.length,
            row;
    ...
    self.chain = function ( chain )
    {
        if ( chained )
            throw new Error ( "Something's already chained to self view." );
