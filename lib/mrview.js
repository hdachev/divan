...
module.exports = function ( mapper, reducer, index, cache )
{
    ...
    /**profile**/
    var rtime = 0, rcount = 0, redfunc = reducer;
    ...
    this.add = function ( doc )
    {
        ...
        mapper ( doc, function ( k, v )
        {
            ...
            if ( chained && warm )
                this.query ({ key : k, group : true }).rows.forEach ( function ( row )
                {
    ...
    this.query = function ( options )
    {
        ...
        ////    Chained map-reduce.
        ////    Start pushing grouped reduce values to a chained component only when warmed up.
    ...
    this.warmUp = function ()
    {
        ...
        delete this.warm;
        var rows = this.query ({ group : true }).rows,
            i, n = rows.length,
            row;
    ...
    {
        if ( chained )
            throw new Error ( "Something's already chained to this view." );
