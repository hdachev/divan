...
module.exports = function ( mapper, reducer, index, cache )
{
    ...
    this.query = function ( options )
    {
        ...
        else
        {
            ...
            {
                count  = index.getLength ();
                ranges = [ index ];
            }

            /** TODO
             *  if ( group ) split ranges
             */
        }
        ...
        {
            if ( reduce )
                out.push ({ key : ( keys && keys [ i ] ) || null, value : reduceRows ( rows, reducer, cache ) });
        ...
        /** TODO
         *  if ( !group ) re-reduce range results
         */
        ...
        return { rows : out, matches : count, select : t1 - t0, render : t2 - t1, output : t3 - t2, total : t3 - t0 };
    };
};
